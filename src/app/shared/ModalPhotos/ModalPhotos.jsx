      ...
      >
        ...
        >
          <ul className="glide__slides h-full ">
            {imgs.map((item, index) => (
              <li className="glide__slide relative h-full flex items-center justify-center" key={index}>
                <NcImage
                
                  src={item}
                  containerClassName=" w-max h-max flex items-center justify-center "
        ...
        <div className="xl:absolute z-20 xl:-inset-x-20 max-w-6xl my-2 mx-auto top-full xl:top-1/2 transform xl:-translate-y-1/2 flex xl:justify-between glide__arrows">
          <NextPrev
            onlyPrev
            className="mr-1.5 "
            btnClassName="w-8 h-8 sm:w-10 sm:h-10 text-black bg-white "
          />
          <NextPrev
            onlyNext
            className="ml-1.5 "
  ...
  const renderModalPhotos = () => {
    return (
      <Dialog
        ...
        as="div"
        className="ProductDetailModalPhotos fixed inset-0 z-50 overflow-y-auto dark bg-black/80 backdrop-blur text-neutral-200 hiddenScrollbar"
        onClose={onClose}
        open={isOpen}
      >
        <div className="min-h-screen px-4 text-center">
          ...
          >
            <ButtonClose className=" sm:w-11 sm:h-11 text-white bg-white/40" onClick={onClose} />
          </div>
